...
=======
| **Designsystem** | **Arkiv v2.1** — papper `#F4EDE1`, bläck `#1F1A15`, Bordeaux `#7A2E2E`; Cormorant Garamond + Inter Tight + JetBrains Mono |
>>>>>>> Stashed changes
...
=======
- **Arkiv v2.1**-designsystem (se nedan)
>>>>>>> Stashed changes
...
=======
## § Designsystem för Historia 1a1 — Arkiv v2.1

Kursen Historia 1a1 använder **Arkiv v2.1** (samma som kursens slides-skill `/slides`, etablerat i kursminnet 2026-04-23). Källa: `.claude/skills/slides/SKILL.md` och `.claude/skills/slides/references/slides.jsx` — den senare är source-of-truth för exakta mått.

### Färgpalett

| Roll | Namn | Hex | Användning |
|------|------|-----|------------|
| Bakgrund | Papper | `#F4EDE1` | Standardbakgrund (alla material) |
| Bakgrund alt. | Papper 2 | `#EBE1CF` | Informationsrutor, sekundära ytor |
...
| Accent 2 | Marin | `#2C3E55` | Fakta, analys, statistik |
| Accent 3 | Oliv | `#5A6A3A` | Reflektion, "bra exempel" |
...
|------|------|---------|------|
| Rubrik H1 (cover) | Cormorant Garamond | 116 px | 600 |
| Rubrik H1 (section) | Cormorant Garamond | 96 px | 600 |
| Rubrik H1 (content) | Cormorant Garamond | 72 px | 600 |
| Citat | Cormorant Garamond | 62 px | 600 italic |
| Ingress / H2 | Cormorant Garamond | 32–36 px | 500 italic |
| Eyebrow / H3 | JetBrains Mono | 18 px | 700 versaler, 2 px letterspacing |
| Brödtext | Inter Tight | 28 px (line-height 40) | **500** |
| Bullets | Inter Tight | 28 px | 500 (index 24 px JetBrains Mono 700) |
| Metadata | JetBrains Mono | 15–17 px | 600 versaler |
| Citat-attribution | Inter Tight | 30 px | 700 i Ocker |

### Designprinciper (från slides-skillen)

1. **Max 3 accentfärger per slide/dokument.** Bordeaux är signatur.
2. **Max 2 betonade ord per rubrik**, max 5 per stycke. Verktyg: `<em>` kursiv, `<strong>` bold, `<mark>` ocker-highlight, `<u>` bordeaux-understreck.
3. **Rubriker bär mening** — frågor som rubrik är systemets signaturdrag (t.ex. "*Hur* blev det möjligt?").
4. **Max 3 nyckelpunkter per slide.**
5. **Inga emojis.** Typografiska tecken: `▸ ● ▪ § № ›`.
6. **Citera källor** — Arkiv är ett källkritiskt system.
7. **Brödtextbredd:** max 32 em.

### Flashcards (kursspecifik konvention, bevaras)

Landskaps-A4, 2×2-rutnät, baksidor måste byta rader (rad 1↔rad 2), kolumner behålls. Motsvarar **långsides-flip**. Titelsida: "vänd på långsidan". Färgsättning: papper-bakgrund med bordeaux/marin accenter för begrepp.

### Word-dokument (.docx)

Skill `/docx` har inget Arkiv-tema inbyggt — för Word-dokument approximera Arkiv genom:
- Brödtext: Inter Tight (eller Calibri som fallback om Inter saknas), 11–12pt
- Rubriker: Cormorant Garamond (eller Georgia som fallback) i Bordeaux `#7A2E2E`
- Bakgrund: vit (Papper är HTML/slide-specifikt)
- Markeringar: Ocker `#B8862F` för highlights, Marin `#2C3E55` för faktarutor
>>>>>>> Stashed changes
...
**Word-specifikation** (Arkiv v2.1-approximation i Word — per `.claude/skills/docx/SKILL.md`):
...
=======
- Rubriker som Heading 1/Heading 2 i **Cormorant Garamond** (fallback: Georgia) i Bordeaux `#7A2E2E`
- Brödtext **Inter Tight** 11–12pt (fallback: Calibri) i Bläck `#1F1A15`
- Eyebrows / metadata: **JetBrains Mono** 10pt versaler i Bläck 2 `#4A3F33`
- Highlights med Ocker `#B8862F` bakgrundsfärg, faktarutor med Marin `#2C3E55` ram
>>>>>>> Stashed changes
- Tidsplaneringen som riktig tabell (DXA, ej percentage)
- Bullets med `LevelFormat.BULLET`
- Sidfot: "Historia 1a1 · Andra världskriget och Förintelsen · Lektion N" (JetBrains Mono versaler)
...
=======
⚠ **Använd `/slides`-skillen** — kursen kör Arkiv v2.1 sedan 2026-04-23. Skillen är Arkiv-native och är source-of-truth för slide-formatet.

Format: **Self-contained HTML** via `/slides`-skillen. Inte .pptx (slides-skillen genererar HTML; pptx-skillen ska INTE användas för detta moment). Referenskomponenter i `.claude/skills/slides/references/slides.jsx`.

Tidigare Arkiv-presentation att referera till (samma kurs, samma moment, redan godkänd):
```bash
ls "/home/anders/Second brain/Historia/Andra världskriget och Förintelsen/presentation-lektion-1.html"
```
>>>>>>> Stashed changes
...
=======
- Bakgrund Papper `#F4EDE1`, text Bläck `#1F1A15`
- **Bordeaux `#7A2E2E`** som signaturfärg på rubriker
- Cormorant Garamond rubriker (72 px content, 96 px section, 116 px cover)
- Inter Tight 28 px vikt 500 för brödtext
- JetBrains Mono för metadata (15–17 px, versaler, 600 vikt)
- Max 3 accentfärger per slide — Bordeaux + två av Marin/Oliv/Ocker/Kritblå/Mossgrön/Tegel
- Max 2 betonade ord per rubrik, max 5 per stycke (`<em>` / `<strong>` / `<mark>` ocker / `<u>` bordeaux)
- Frågor som rubrik är systemets signaturdrag
- **INGA emojis** — typografiska tecken (▸ ● ▪ §) får användas
- Diskussionspaus var 3–4:e slide (slide-typ `discuss` eller `question`)
- Familjefoton + kartor framför grafiska bilder
- Källhänvisningar på fakta-slides (Arkiv är ett källkritiskt system)
- Slide-typer: `cover`, `section`, `question`, `content`, `content-highlight`, `quote`, `data`, `timeline`, `discuss`, `callout`, `close`
>>>>>>> Stashed changes

Spara: `Undervisningsmaterial/Historia/Andra världskriget och Förintelsen/presentation-lektion-N.html`
...
=======
**Använd Arkiv v2.1-paletten** — Papper `#F4EDE1` bakgrund, Bläck `#1F1A15` text, Bordeaux `#7A2E2E` rubriker/accent, Cormorant Garamond rubriker, Inter Tight brödtext. Override skillens default historiapalett om sådan finns.
>>>>>>> Stashed changes
...
=======
- Designsystem (Arkiv v2.1): Papper-bakgrund `#F4EDE1` med Bordeaux `#7A2E2E` rubriker/ramar och Marin `#2C3E55` eller Ocker `#B8862F` som sekundär accent. Cormorant Garamond på begreppstermerna (framsidan), Inter Tight på svaren (baksidan).
>>>>>>> Stashed changes
